...
size_t DataPkg::toBytes(char* buf, size_t buflen) {
	...
	uint32_t parInts[_INT_CNT] = {time, temp, pressure, bmpTemp, battery, tempDS};
...
*/	
	...
	printf("Temperature: %.3lf @Pi, %.3lf @DS, %.1lf @BMP\n", (temp*0.001),
			(tempDS*0.001), (bmpTemp*0.1));
//	printf("Humidity: %.1lf\n", dhtHum*0.1);
