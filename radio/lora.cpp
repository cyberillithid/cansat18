...
//endif
	...
	{ // FSK mode
		throw std::runtime_error("LORA installation failed");
	}
}
...
void LoRa::fetchSyncWord() {
	...
	if (config1==0x12) {
		...
		throw std::runtime_error("Sync word setting failure");
	}
...
void LoRa::setMode(){ //using 'mode 1'
	...
	*/
	...
	if (config1 != 0x98) //(config1 >> 1) != 0x39 )
		throw std::runtime_error("Error setting mode");
...
void LoRa::setChannel(uint32_t freq) {
	...
	writeRegister(REG_OP_MODE, st0);
	if (f != freq) 
		throw std::runtime_error("Frequency set failed");
...
void LoRa::setPower() {
	...
	if (readRegister(REG_PA_CONFIG) != 0xFF) //MAX POWER (ex. 0x7F)
		throw std::runtime_error("Power cfg failed");
	writeRegister(REG_OP_MODE, st0);
}
